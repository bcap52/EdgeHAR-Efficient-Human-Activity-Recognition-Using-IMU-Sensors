function [Dataset_50HZ_Limited_Train_19974_eachclass, Dataset_50HZ_Limited_Test_Final, mobile_features] = Data_PreProcessing_FeatureEngineering()
...
%  PART 3: MOBILE DATA PREPARATION

    % Setting up the path relative to the current folder
    if isfolder('Data/Mobile Recordings')
        mobileFolder = 'Data/Mobile Recordings';
    elseif isfolder('data/Mobile Recordings')
        mobileFolder = 'data/Mobile Recordings';
    else
        % Fallback manual selection
        mobileFolder = uigetdir(pwd, 'Select the Mobile Recordings folder');
    end
    fprintf('Processing MATLAB Mobile Data from: %s\n', mobileFolder);
    ...
    % initialisation noise when the recording begins
    thigh_x_raw = Acceleration.X(351:end);
    thigh_y_raw = Acceleration.Y(351:end);
    thigh_z_raw = Acceleration.Z(351:end);

    %Now as our dataset is at 50HZ meaning 50 readings per second, so one row will not have enough time's worth of data to determine the activity, in order to fix this we need to have more time's data in a single row. We will window our data for this, our goal is to achieve 2.5 seconds worth of data in one row so since 50 rows amount to a second of data, we would need to average out 125 rows into one feature vector.
    %We will also increase our features from 6 to 12, as 6 features would be the mean of all the 125 rows's features and the other 6 would be the standard deviation as that helps us in determine classes like walking more. Extra physical features=more accuracy, we are doing the best with what quality of the data we have
    ...
    % The training dataset used two IMUs (back + thigh). Since MATLAB Mobile only
    % provides a single IMU, the phone data is mapped to the thigh sensor while
    % back sensor features are set to zero to preserve the expected 12-column feature structure
    %num_windows = size(thigh_x_A, 2);
    back_x_A = thigh_x_A;
    back_y_A = thigh_y_A;
    back_z_A = thigh_z_A;
    fA = [mean(back_x_A)',  mean(back_y_A)',  mean(back_z_A)', ...
          mean(thigh_x_A)', mean(thigh_y_A)', mean(thigh_z_A)', ...
          std(back_x_A)',   std(back_y_A)',   std(back_z_A)', ...
    ...
    mobile_features = array2table(features_A, ...
        'VariableNames', {'back_x_mean', 'back_y_mean', 'back_z_mean', ...
                          'thigh_x_mean', 'thigh_y_mean', 'thigh_z_mean', ...
                          'back_x_std',   'back_y_std',   'back_z_std', ...
                          'thigh_x_std',  'thigh_y_std',  'thigh_z_std'});
